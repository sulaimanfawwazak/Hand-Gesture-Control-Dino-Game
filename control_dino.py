  ...
  if results.multi_hand_landmarks:
    for hand_landmarks in results.multi_hand_landmarks:
      ...
      for idx, landmark in enumerate(hand_landmarks.landmark):
        if idx == 4:
          cx_thumb, cy_thumb = int(landmark.x * frame_width), int(landmark.y * frame_height)
          # print(f'idx {idx}: {cx_thumb}, {cy_thumb}')
          
        if idx == 8:
          cx_index, cy_index = int(landmark.x * frame_width), int(landmark.y * frame_height)
          # print(f'idx {idx}: {cx_index}, {cy_index}')
      ...
      # if distance_y < distance_threshold and current_time - last_jump_time > jump_cooldown:
      if distance_y < distance_threshold:
        auto.press('up')
        print('Jump!')
